  ...
  var ashade_landing = {
    get_event: function () {
      ...
      if (window.location.href.indexOf('?event=') > -1) {
        ...
        window.location.href.indexOf('?page_id=') > -1 &&
...
ashade.link_exception = function ($this) {
  ...
  let classes = ['comment-reply-link', 'ajax_add_to_cart', 'remove'],
    ids = ['cancel-comment-reply-link'],
    result = false
...
class Ashade_Filtered_Grid {
  constructor(this_obj) {
    ...
    this.$filter_mobile_wrap = jQuery(
      '<div class="ashade-mobile-filter-wrap"/>'
    ).appendTo(this.$filter)
    this.$filter_mobile = jQuery(
      '<div class="ashade-mobile-filter"/>'
    ).appendTo(this.$filter_mobile_wrap)
    this.$filter_mobile.append(
      '<span class="ashade-mobile-filter-label">' +
        this.$filter.data('label') +
        '</span>'
    )
    this.$filter_mobile_value = jQuery(
      '<span class="ashade-mobile-filter-value">' +
        this.$filter.find('a.is-active').text() +
        '</span>'
    ).appendTo(this.$filter_mobile)
    this.$filter_mobile_list = jQuery('<ul class="ashade-mobile-filter-list"/>')
      .insertAfter(this.$filter_mobile)
      .slideUp(1)
    ...
    this.$filter.children('a').each(function () {
      let $this_a = jQuery(this),
        this_li = jQuery(
          '<li data-category="' +
            $this_a.data('category') +
            '">' +
            $this_a.text() +
            '</li>'
        ).appendTo(this_class.$filter_mobile_list)
  ...
  set(category, label) {
    ...
    this_class.$el
      .children('div:not(' + category + ')')
      .addClass('ashade-grid-item-hidden')
  ...
  layout() {
    let this_class = this,
      ...
      spacing_x =
        parseInt($items.css('margin-left'), 10) +
        parseInt($items.css('margin-right'), 10),
      spacing_y =
        parseInt($items.css('margin-top'), 10) +
        parseInt($items.css('margin-bottom'), 10),
    ...
    $items.each(function () {
      ...
      $this.css(
        'transform',
        'translate3d(' + setX + 'px, ' + setY + 'px, 0) scale(1,1)'
      )
        ...
        if ('adjusted' == this_class.type) {
          ...
          jQuery(current_row).each(function () {
            ...
            if ($current.height() < rowItemHeight) {
              ...
              $current.css(
                'transform',
                'translate3d(' + this.xPos + 'px, ' + newY + 'px, 0) scale(1,1)'
              )
            }
          })
    ...
    if (
      $ashade_body.hasClass('ashade-home-template') &&
      $ashade_body.hasClass('ashade-smooth-scroll')
    ) {
...
ashade.cursor = {
  ...
  init: function () {
    ...
    jQuery(document)
      ...
      .on(
        'mouseenter',
        '.shadowcore-testimonials-carousel .tns-inner',
        function () {
          ashade.cursor.$el.addClass('int-grab-h')
          jQuery(this).on('mouseleave', function () {
            ashade.cursor.$el.removeClass('int-grab-h')
...
// Lightbox
if (
  jQuery('.ashade-lightbox-link').length ||
  jQuery('.shadowcore-lightbox-link').length
) {
  ashade.pswp = {
    gallery: Array(),
    html: jQuery(
  ...
  preloadImage(this_img) {
    ...
    this_img.addEventListener('load', function (e) {
      ...
      if (navigator.userAgent.toLowerCase().indexOf('firefox') > -1) {
        if (
          jQuery(this_img).parents('div').hasClass('ashade-justified-gallery')
        ) {
          setTimeout(
            function () {
          ...
          setTimeout(
            function () {
              jQuery(this_img)
                .parents('.ashade-justified-gallery')
                .justifiedGallery()
            },
            500,
            this_img
    ...
    animate: function () {
      ...
      if (jQuery('.wp-block-cover-image.has-parallax').length) {
        jQuery('.wp-block-cover-image.has-parallax').each(function () {
          let $this = jQuery(this),
            ...
            current_scroll =
              ($ashade_window.scrollTop() - this_offset_top) / 100,
      ...
      if (!$ashade_body.hasClass('ashade-home-template')) {
        if ($ashade_body.hasClass('admin-bar')) {
          if (
            $ashade_scroll.height() !==
            $ashade_body.height() + jQuery('#wpadminbar').height()
          ) {
            ashade.sScroll.layout()
          }
        } else {
    ...
    layout: function () {
      ...
      if ($ashade_scroll.length) {
        ...
        if ($ashade_scroll.height() <= $ashade_window.height()) {
          ...
          if ($ashade_body.hasClass('ashade-layout--horizontal')) {
            if (jQuery('.ashade-to-top-wrap.ashade-back-wrap').length) {
              min_height =
                min_height -
                jQuery('.ashade-to-top-wrap.ashade-back-wrap').height()
            }
            if (jQuery('.ashade-page-title-wrap').length) {
              min_height =
                min_height - jQuery('.ashade-page-title-wrap').height()
            }
          }
        ...
        if ($ashade_window.width() > 960) {
          $ashade_body.height(
            $ashade_scroll.height() - jQuery('#wpadminbar').height()
          )
        } else {
...
ashade.init = function () {
  ...
  if (
    ashade.config.referrer !== '' &&
    ashade.config.referrer !== ashade.config.location &&
  ...
  if (jQuery('.ashade-protected-form-wrap').length) {
    let $protected_wrap = jQuery('.ashade-protected-form-wrap'),
      ...
      $protected_form_inner = jQuery(
        '<div class="ashade-protected-form-inner">'
      ).appendTo($protected_form),
      $password_input = $protected_form
        .find('label > input')
      ...
      $password_submit = $protected_form
  ...
  let ashade_mobile_header = jQuery('<div class="ashade-mobile-header">'),
    mobile_menu_button = jQuery(
      '<a href="javascript:void(0)" class="ashade-mobile-menu-button"><i class="la la-bars"></i></a>'
  ...
  if ($ashade_header.find('.ashade-wc-header-cart').length) {
    ashade_mobile_header.prepend(
      $ashade_header.find('.ashade-wc-header-cart').clone()
    )
  }
  ...
  if ($ashade_body.hasClass('ashade-maintenance-wrap')) {
    ashade_mobile_header.prepend(
      '<a class="ashade-contacts-toggler" href="javascript:void(0)"><i class="la la-envelope"></i></a>'
    )
          ...
          },
          {
            ...
            onComplete: function () {
              if ($ashade_body.find('.ashade-content-scroll').length) {
                var $scroll_content = $ashade_body.find(
        ...
        if (jQuery('.ashade-grid.has-filter:not(.is-masonry)').length) {
          jQuery('.ashade-grid.has-filter:not(.is-masonry)').each(function () {
            if (ashade_f_grid[jQuery(this).attr('id')]) {
              ...
              ashade_f_grid[jQuery(this).attr('id')] = new Ashade_Filtered_Grid(
                this
              )
            }
          })
          ...
          if ($this.attr('target') && '_blank' == $this.attr('target')) {
            ...
            if (!ashade.link_exception($this)) {
              ...
              if (this_href == '#') {
                ...
                $this.hasClass('ashade-lightbox-link') ||
                $this.hasClass('shadowcore-lightbox-link')
              ) {
                e.preventDefault()
              } else if (
                this_href.length > 1 &&
    ...
    jQuery('.is-masonry').each(function () {
      ...
      if ($this.hasClass('has-filter')) {
        let $filter = $this.prev('div.ashade-filter-wrap'),
          $filter_mobile_wrap = jQuery(
            '<div class="ashade-mobile-filter-wrap"/>'
          ).appendTo($filter),
          $filter_mobile = jQuery(
            '<div class="ashade-mobile-filter"/>'
          ).appendTo($filter_mobile_wrap),
          $filter_mobile_list = jQuery(
            '<ul class="ashade-mobile-filter-list"/>'
          )
            .insertAfter($filter_mobile)
            .slideUp(1),
          $filter_mobile_value = jQuery(
            '<span class="ashade-mobile-filter-value">' +
              $filter.find('a.is-active').text() +
              '</span>'
          ).appendTo($filter_mobile),
  ...
  if ($ashade_body.find('select').length) {
    $ashade_body.find('select').each(function () {
      if (
        jQuery(this).parent().hasClass('woocommerce-input-wrapper') &&
        jQuery(this).parents('form').hasClass('woocommerce-checkout')
      ) {
      ...
      $this
        .wrap('<div class="ashade-select-wrap"/>')
        .after($list)
        .after($select)
        .addClass('is-hidden')
      ...
      $select.on('click', function (e) {
        ...
        // Close Other Lists
        jQuery('div.ashade-select-wrap.is-active')
          .not($thisWrap[0])
          .each(function () {
  ...
  if ($ashade_body.find('input[type="radio"]:not(".is-default")').length) {
    $ashade_body.find('input[type="radio"]').each(function () {
      ...
      $this
        .wrap(
          '<div class="ashade-radio-wrap is-link ' + check_class + '"></div>'
        )
        .on('click', function () {
          jQuery('[name="' + jQuery(this).attr('name') + '"]').each(
            function () {
  ...
  if ($ashade_body.find('input[type="checkbox"]').length) {
    $ashade_body.find('input[type="checkbox"]').each(function () {
      ...
      if (
        !$checkbox
          .parent('label')
          .hasClass('woocommerce-form__label-for-checkbox')
      ) {
        ...
        $checkbox
          .wrap(
            '<div class="ashade-checkbox-wrap is-link ' +
              check_class +
              '"></div>'
          )
          .on('click', function () {
  ...
  // Back to Top
  if (
    jQuery('.ashade-back.is-to-top').length &&
    $ashade_body.hasClass('ashade-layout--horizontal')
  ) {
    $ashade_body.addClass('has-to-top')
  }
    ...
    jQuery('.ashade-author-notify-button').on('click', function () {
      ...
      jQuery.post(
        ...
        function (response) {
          $this.slideUp(300)
          $parent
            .find('.ashade-client-notify-message')
            .html(response)
            .slideDown(300)
          setTimeout(
            function () {
  ...
  if (jQuery('.ashade-attachment-wrap').length) {
    jQuery('.ashade-attachment-wrap').each(function () {
      let $this = jQuery(this),
        ...
        maxH =
          $ashade_window.height() -
          $ashade_header.height() -
          $ashade_footer.height() -
          parseInt($inner.css('padding-top'), 10) -
          parseInt($inner.css('padding-bottom'), 10),
        maxW =
          $ashade_window.width() -
          parseInt($inner.css('padding-left'), 10) -
          parseInt($inner.css('padding-right'), 10),
        tempW = maxH / ratio,
        setW,
        setH
      if ($ashade_body.hasClass('admin-bar')) {
...
ashade.loading = function () {
  if ($ashade_body.hasClass('ashade-loading--full')) {
    ...
    if (!$ashade_body.hasClass('ashade-layout--horizontal')) {
      ...
      if (jQuery('.ashade-page-title-wrap:not(.is-inactive)').length) {
        if (
          jQuery('.ashade-page-title-wrap:not(.is-inactive)').hasClass(
            'ashade-page-title--is-alone'
          )
        ) {
          setTimeout(
    ...
    if (
      ashade_landing &&
      'contacts' !== ashade_landing.get_event() &&
      'works' !== ashade_landing.get_event()
    ) {
  ...
  if ($ashade_body.hasClass('ashade-unloading--none')) {
    ...
    if (!$ashade_body.hasClass('ashade-layout--horizontal')) {
      ...
      if (jQuery('.ashade-back-wrap:not(.is-inactive)').length) {
        ...
        gsap.to(
          '.ashade-back-wrap:not(.is-inactive):not(.ashade-404-return-wrap)',
          0.5,
          {
            css: {
              x: '100%'
        ...
        gsap.to(
          '.ashade-back-wrap.ashade-404-return-wrap:not(.is-inactive)',
          0.5,
          {
            css: {
              x: '-100%'
